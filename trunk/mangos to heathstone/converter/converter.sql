...
drop table if exists mangos_strinig;
...
drop table if exists gamobject_respawn;
...
ALTER TABLE `creature_model_info` ADD `entry` INT ( 10) NOT NULL default '0' AFTER `modelid`;
update creature_model_info set entry=(select entry from creature_names where male_displayid=creature_model_info.modelid);
update creature_proto set combat_reach=(select combat_reach from creature_model_info where entry=creature_proto.entry);
update creature_proto set bounding_radius=(select bounding_radius from creature_model_info where entry=creature_proto.entry);
update creature_proto set equipmodel1=(select equipentry1 from creature_equip_template where entry=creature_proto.entry);
update creature_proto set equipmodel2=(select equipentry2 from creature_equip_template where entry=creature_proto.entry);
update creature_proto set equipmodel3=(select equipentry3 from creature_equip_template where entry=creature_proto.entry);
drop table creature_omodel_info;
...
REPLACE creature_waypoints (spawnid, waypointid, position_x, position_y, position_z, waittime)
SELECT id, point, position_x, position_y, position_z, waittime
FROM creature_movement;

drop table creature_movement;
...
REPLACE reputation_creature_onkill (creature_id, faction_change_alliance, faction_change_horde, change_value)
SELECT creature_id, RewOnkillRepFaction1, RewOnKillRepFaction2, RewOnKillRepValue1 
FROM creature_onkill_reputation;

drop table creature_onkill_reputation;
